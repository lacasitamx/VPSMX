...
install_base() {
    if [[ x"${release}" == x"centos" ]]; then
        yum install wget curl tar unzip -y && echo -e "\033[1;32m [ EXITO ]" || echo -e "\033[1;31m [FAIL]"
    else
        apt install wget curl tar unzip -y && echo -e "\033[1;32m [ EXITO ]" || echo -e "\033[1;31m [FAIL]"
    fi
}
...
install_v2-ui() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -q -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://github.com/sprov065/v2-ui/releases/download/${last_version}/v2-ui-linux.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        wget -q -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
